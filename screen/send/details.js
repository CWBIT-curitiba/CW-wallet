...
export default class SendDetails extends Component {
  ...
  async createTransaction() {
    ...
    const firstTransaction = this.state.addresses.find(element => {
      const feeSatoshi = new BigNumber(element.amount).multipliedBy(100000000);
      return element.address.length > 0 && (feeSatoshi > 0 || element.amount === BitcoinUnit.MAX);
    });
    if (!firstTransaction) {
      error = 'Unable to find a valid recipient. Please enter an amount and a valid address.';
    } else {
